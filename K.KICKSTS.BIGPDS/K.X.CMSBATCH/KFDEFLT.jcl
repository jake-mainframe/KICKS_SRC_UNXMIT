...
void stackit(char *catuserid,
 ...
 char lineÝ80¨, *aline=&lineÝ0¨; int alinel;
 // clear stack
...
int main (int argc, char *argvÝ¨) { // kiksip is main...
KIKCSA *csa;
int i; char *ip, firstnlÝ8¨; long L;
fcth *fcthdr;
 ...
 for (i = 1; i < argc; i++)
  if (!memcmp(argvÝi¨, "SIT=", 4))
   if ((*(argvÝi¨+4) != 0) && (*(argvÝi¨+4) != ' ')) {
   memcpy(&csa->sit_table_suffix, (argvÝi¨)+4, 2);
 ...
 if (memcmp((char *)&csa->version, &fcthdr->verÝ0¨, 4)) {
