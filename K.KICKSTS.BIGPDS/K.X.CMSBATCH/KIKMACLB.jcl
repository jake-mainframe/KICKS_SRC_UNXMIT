...
char  CMSeofmkÝ4¨={0,0,0,0}; // EOF value after first open
#define NUMFSCBS 10
fscb  CMSfscbÝNUMFSCBS¨;

fscb *CMSopen (char *name) { // NULL open failed, else OK
 int i, r15=0; fscb *ModelFSCB;
      // first time setup eofmark & clear fscb table
      if (CMSeofmkÝ0¨ == 0) {
        CMSeofmkÝ0¨=CMSeofmkÝ3¨=0x61;  // keep actual 61ffff61
        CMSeofmkÝ1¨=CMSeofmkÝ2¨=0xff;  // out of the code...
        for (i=00; i<NUMFSCBS; i++) {
         memset(CMSfscbÝi¨, 0, sizeof(fscb));
         }
        }
      // find free slot
      for (i=0; i<NUMFSCBS; i++) {
       if (CMSfscbÝi¨.fscbfnÝ0¨ == 0) break;
       }
      ...
      memcpy(CMSfscbÝi¨, ModelFSCB, 44);
      // copy file name to slot name
      memcpy(&CMSfscbÝi¨.fscbfn, name, 18);
      ...
      // save last return
      CMSfscbÝi¨.CMSr15 = r15;
      // if open good return fscb pointer
      if (r15 == 0) return &CMSfscbÝi¨;
...
int   CMSeof  (fscb *fscb) {  // 0 ok, 12 eof, else othr error...
 int i;
      ...
      if(i == NUMFSCBS) return -1;
      // fail call if slot not open
      if (CMSfscbÝi¨.fscbfnÝ0¨ == 0) return -1;
      // return eof flag
      return fscb->CMSr15;
 }
...
void  CMSclose(fscb *fscb) {  // close (if open)
 int i, r15=0;
      ...
      // fail call if slot not open
      if (CMSfscbÝi¨.fscbfnÝ0¨ == 0) return;
      // close file
      __asm__ (
        "L 2,%1\n\t"
        "FSCLOSE FSCB=(2)\n\t"
      ...
      // make slot free
      memset(fscb, 0, sizeof(CMSfscbÝ0¨));
 }
...
int kikmaclb (int whycalled,
 ...
 static char maclibsÝ72¨;
 static char *MACLIBS=&maclibsÝ0¨;
 ...
 dirptr *dp; dir *drec;
 fscb *inc;
 char incnameÝ22¨, *maclibname, newlineÝ90¨;
 int  i, j, k;
 ...
 if (modtab == NULL) { // 1st time initialization
  ...
  { char conbufÝ132¨; int i;
   ...
   memcpy(MACLIBS, &conbufÝ11¨, 72);
   for (i=8; i< 73; i+=9) MACLIBSÝi¨ = 0;
    ...
    while (1) {
     ...
     for (k=0; k<6; k++) {
      if (drec->memsÝk¨.memberÝ0¨ != 0) {
       // active entry
       j--; // reduce count of active entries
       memcpy(modtabÝnummods¨.member, drec->memsÝk¨.member, 8);
       modtabÝnummods¨.lib = i;
       modtabÝnummods¨.recnum = drec->memsÝk¨.firstblock;
       nummods++; // increase count of mods
       }
      if(j < 1) break; // for
      }
 ...
 switch (whycalled) {
  ...
  case 2:  // open member
     ...
     for (i=0; i<nummods; i++) {
      if(!strncmp(memname, modtabÝi¨.member, 8)) break;
      }
